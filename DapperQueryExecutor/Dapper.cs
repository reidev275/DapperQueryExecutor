...
namespace DapperQueryExecutor
{
	...
	/// Modified existing scope from public to force usage of command pattern
	/// </summary>
...
#pragma warning restore 618
		...
		public static List<PropInfo> GetSettableProps(Type t)
		{
			return t
				...
				{
					Name = p.Name,
					ColumnAlias = (p.GetCustomAttributes(typeof(ColumnAttribute), false).Count() == 1) ? (p.GetCustomAttributes(typeof(ColumnAttribute), false).FirstOrDefault() as ColumnAttribute).Name : p.Name,
...
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
...
)
		{
			...
			var setters = (
					from n in names
					let prop = properties.FirstOrDefault(p => string.Equals(p.ColumnAlias, n, StringComparison.Ordinal)) // property case sensitive first
						?? properties.FirstOrDefault(p => string.Equals(p.ColumnAlias, n, StringComparison.OrdinalIgnoreCase)) // property case insensitive second
...
string name, object value = null, DbType? dbType = null, ParameterDirection? direction = null, int? size = null
	...
	public static class SqlMapperExtensions
	{
		...
		public static IEnumerable<PropertyInfo> KeyPropertiesCache(Type type)
		{
			...
			if (keyProperties.Count == 0)
			{
				var idProp = allProperties.FirstOrDefault(p => p.Name.ToLower().EndsWith("id"));
				if (idProp != null)
				{
					keyProperties.Add(idProp);
				}
		...
		public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
		{
			...
			if (!GetQueries.TryGetValue(type.TypeHandle, out sql))
			{
				...
				var keyName = onlyKey.GetCustomAttributes(typeof(SqlMapper.ColumnAttribute), false).Count() == 1 ? (onlyKey.GetCustomAttributes(typeof(SqlMapper.ColumnAttribute), false).FirstOrDefault() as SqlMapper.ColumnAttribute).Name : onlyKey.Name;

				// TODO: pluralizer 
				// TODO: query information schema and only select fields that are both in information schema and underlying class / interface 
				sql = "select " + String.Join(",", GetColumnsFromType(type)) + " from " + name + " where " + keyName + " = @id";
		...
		private static string[] GetColumnsFromType(Type type)
		{
			var props = SqlMapper.GetSettableProps(type);
			return props.Select(x => String.IsNullOrEmpty(x.ColumnAlias) ? x.Name : x.ColumnAlias).Select(x => String.Format("[{0}]", x)).ToArray();
		}
